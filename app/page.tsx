        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {newMarketProperties.map((property, index) => {
              // Hide some items based on screen size as done in original HTML for a responsive grid:
              // Article 5: hidden xl:flex
              // Article 6: hidden lg:flex
              let additionalClasses = "";
              if (index === 4) additionalClasses = "hidden xl:flex";
              if (index === 5) additionalClasses = "hidden lg:flex";
              
              return (
                <div key={property.id} className={`${additionalClasses}`}>
